...
If you're using React, the HTML snippets on the demo page work with a few small JSX changes:

- Use `className` instead of `class`
- Pass `style` as an object
- Keep the animation styles in your app stylesheet by copying `css/corner.css`
...
For the left corner variant, switch `right: 0` to `left: 0` and add `transform: "scale(-1, 1)"` to the `<svg>` style object.
